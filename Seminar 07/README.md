...
- **rvalue** е временна стойност (_prvalue_ + _xvalue_)
...
Понякога се използват и термините **prvalue** и **xvalue**:

- `5` е **prvalue**
- `std::move(x)` е **xvalue**

И в двата случая говорим за **rvalue**.

Може да се мисли така:

- **prvalue**: временна стойност
- **xvalue**: специален вид rvalue — стойност, която все още е свързана с конкретен обект, но от която вече е позволено да се „премести“. Точно такъв резултат връща `std::move(obj)`.
...
>Идеята на функцията std::move е да играе ролята на каст от lvalue към xvalue, но въпреки това, тя може да приема като аргументи не само lvalue обекти, ами и rvalue такива. Примерно: `std::move(std::move(x))`
