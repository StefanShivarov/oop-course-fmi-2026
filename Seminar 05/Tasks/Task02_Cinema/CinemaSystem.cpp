...
void CinemaSystem::resizeScreenings(unsigned newCapacity) {
    Screening* newScreenings = new Screening[newCapacity];
    ...
    screeningsCapacity = newCapacity;
}
...
CinemaSystem::CinemaSystem(): screeningsCount(0), screeningsCapacity(4) {
    screenings = new Screening[screeningsCapacity];
}

CinemaSystem::CinemaSystem(const CinemaSystem& other): screeningsCount(other.screeningsCount), screeningsCapacity(other.screeningsCapacity) {
    copyDynamicMemory(other);
}
...
bool CinemaSystem::addScreening(const Screening& screening) {
    ...
    if (screeningsCount >= screeningsCapacity) {
        resizeScreenings(screeningsCapacity + 4);
    }
...
bool CinemaSystem::removeScreening(unsigned screeningId) {
    for (unsigned i = 0; i < screeningsCount; i++) {
        if (screenings[i].getId() == screeningId) {
            for (unsigned j = i; j + 1 < screeningsCount; j++) {
...
unsigned CinemaSystem::getFreeSeatsCount(unsigned screeningId) const {
    ...
    return screening->getFreeSeatsCount();
}
...
void printAllScreenings(const CinemaSystem& c, std::ostream& os) {
    ...
    for (unsigned i = 0; i < c.getScreeningsCount(); i++) {
        ...
        if (s != nullptr) {
            printScreening(*c.getScreeningAt(i), os);
        }
    }
}
