...
NumberWrapper operator+(const NumberWrapper& lhs, const NumberWrapper& rhs) {
    ...
    result += rhs; // reusing the already implemented operator+=
    return result;
}
...
bool operator==(const NumberWrapper& lhs, const NumberWrapper& rhs) {
    return (int) lhs == (int) rhs; // using conversion operator int()
}
...
bool operator>(const NumberWrapper& lhs, const NumberWrapper& rhs) {
    return rhs >= lhs && lhs != rhs; // reusing the already implemented operator>= and operator!=
}

bool operator<=(const NumberWrapper& lhs, const NumberWrapper& rhs) {
    return !(lhs > rhs); // reusing the already implemented operator>
}
